...
- **The PostgreSQL DDL file** is executed as a single batch (no statement splitting) because it contains a PL/pgSQL `DO` block — the guard that keeps the legacy `lock_version` widening from re-taking an `ACCESS EXCLUSIVE` lock on every startup. Also handled internally; it matters only if you copy that file for `ddl-location` (see below).
...
7. **Statement separator is `;`.** The `ddl-location` path always uses `;` — the `/` separator used internally for the Oracle/DB2 bundled scripts, and the whole-script batching used for the PostgreSQL one, are tied to their `Dialect` enum entries and do **not** apply to user-supplied files. Structure your statements so no `;` appears inside a logical statement (no unguarded PL/SQL / PL/pgSQL / compound-SQL blocks).
...
Quickest path: copy `h2.sql` (the minimal reference) from the library jar, adjust the dialect-specific types (identity, timestamp), and point `ddl-location` at your copy. If you start from `postgresql.sql` instead, delete its trailing `DO $$ … $$;` block first — that block is the internal legacy-widening guard, it is meaningful only for schemas created by this library before 2.0.0, and its embedded semicolons would be split apart by the `;` separator this path uses (rule 7).

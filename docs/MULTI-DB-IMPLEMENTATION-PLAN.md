...
> ## Status: DELIVERED in 2.2.0 — historical record
>
> This is the plan as written on 2026-04-23, **before** implementation. Everything
> it specifies shipped in 2.2.0: the `Dialect` enum, `DialectDetector`, all six DDL
> files, the `ddl-location` property, and the Testcontainers matrix behind the
> `heavy-it` profile. It is kept for the rationale and the scope decisions, not as
> pending work.
>
> **Where the code disagrees with this document, the code is right.** Two claims
> below were disproved during implementation and are corrected inline where they
> appear:
>
> 1. **DB2 LUW does not support `TIMESTAMP WITH TIME ZONE`** at any version. That
>    form is DB2 for z/OS only. The shipped `db2.sql` uses plain `TIMESTAMP`, with
>    the same run-everything-in-UTC caveat as MySQL. (§4.5)
> 2. **"Runtime queries are already SQL-92 compliant, so nothing changes" was too
>    strong.** The *SQL* needed no changes, but Oracle's JDBC driver returns a
>    `ROWID` from `Statement.RETURN_GENERATED_KEYS` rather than the identity
>    column, which forced `JdbcHelper.autoIncrementInsert` to name the `id` column
>    explicitly — with case folding, since Oracle/DB2/H2 upper-case unquoted
>    identifiers. (§1)
>
> Later work has also moved past this document: 2.2.2 relocated the PostgreSQL
> `lock_version` widening out of the DDL into `LockVersionMigration`, so
> `postgresql.sql` no longer carries the `ALTER` statements described here.
...
> **Correction (found during implementation).** "The SQL is portable, therefore the
> Java needs no changes" does not follow, and this paragraph leans on it. The
> statement *text* was indeed portable, but **JDBC behaviour is not part of SQL**:
> Oracle's driver returns a `ROWID` from `Statement.RETURN_GENERATED_KEYS` instead
> of the generated identity value, so `rs.getInt(1)` blew up on the very first
> Oracle integration test. `JdbcHelper.autoIncrementInsert` now names the `id`
> column explicitly via `prepareStatement(sql, String[])`, upper-casing the name
> when `DatabaseMetaData.storesUpperCaseIdentifiers()` reports it — Oracle, DB2 and
> H2 fold unquoted identifiers to upper case, PostgreSQL to lower.
>
> The lesson generalises beyond this one method: **a portability audit of a JDBC
> library has to cover driver behaviour** — generated keys, identifier case,
> `getSchema()`/`getCatalog()` semantics, type mapping — not just the SQL strings.
...
| DB2 LUW 11.x | `GENERATED BY DEFAULT AS IDENTITY` | ~~`TIMESTAMP` with `WITH TIME ZONE` (LUW 11.5+)~~ **plain `TIMESTAMP` — see correction below** | native | ❌ | Wrap in `BEGIN … EXCEPTION WHEN SQLSTATE '42710' THEN NULL; END;` compound SQL block; catches "object already exists" |
...
> **Correction (found during implementation).** The table row above was wrong about
> DB2: **DB2 LUW has no `TIMESTAMP WITH TIME ZONE` at any version**, 11.5 included.
> That syntax exists only in DB2 for z/OS. So DB2 LUW lands in the same bucket as
> MySQL, not the same bucket as Oracle: the shipped `db2.sql` declares plain
> `TIMESTAMP`, and the UTC caveat in point 2 applies to **both** MySQL and DB2.
> The README documents both. Anywhere this plan says the DDL needs DB2 LUW 11.5+
> "for `TIMESTAMP WITH TIME ZONE`", the real reason for a version floor is the
> compound-SQL idempotency guard, not the timestamp type.
...
> **Correction (found during implementation).** The parenthetical is wrong, and so
> is the escape hatch it implies. DB2 LUW never had `TIMESTAMP WITH TIME ZONE`, so
> the shipped `db2.sql` already uses plain `TIMESTAMP` — there is nothing for an
> older DB2 to fall back *from*. The no-tz caveat the README ended up carrying
> covers **MySQL and DB2 alike**, and the 11.5+ floor that shipped is about the
> compound-SQL idempotency guard instead.
